...
<script type="text/javascript">
	...
	function changeIcon(stage,ii) {
		...
		if (currentPossibility=="0") {
			...
			//如果当前阶段可能性不为0，前7个绿圈，绿色标记，黑圈，后两个一定是黑叉
			//遍历前7个，绿圈，绿色标记，黑圈
			for (var i=0; i<point; i++){
				//如果是当前阶段
				if (i==index){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//判断当前阶段
			//如果当前阶段的可能性为0 前7个一定是黑圈 后两个一个是红一个是黑
			if ("0".equals(currentPossibility)){
				for (int i=0; i<dvList.size(); i++){
					//取得每一个遍历出来的阶段， 根据每一个遍历出来的阶段可能性
					DicValue dv = dvList.get(i);
					String listStage = dv.getValue();
					String listPossibility = pMap.get(listStage);

					//如果遍历出来的阶段的可能性为0 说明是后两个，一个红，一个黑
        ...
        <%
				...
				//当前阶段的可能性不为0 前7个可能是绿圈，绿色标记，黑圈， 后两个一定是黑插
			}else {
                    //当前阶段的下标
                int index = 0;
                for (int i=0; i<dvList.size(); i++){
...
<script>
	...
	console.log("<%=currentPossibility%>"+"Possibility");
</script>
