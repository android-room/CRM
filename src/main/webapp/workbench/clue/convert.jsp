...
<script type="text/javascript">
	$(function(){
		...
		//为提交（市场活动） 按钮绑定事件，填充市场活动源（填写两项信息 id+名字）
		...
		$("#convertBtn").click(function () {
			...
			/*
				提交请求到后台，执行转换操作，发出传统请求
				请求结束后，最终影响回线索列表页
			 */
			if ($("#isCreateTransaction").prop("checked")){
				//创建交易，还要传递 金额，预计成交日期，交易日期，交易名称，阶段。市场活动源（id）
...
<%--			${pageContext.request.contextPath}--%>
		-->
		<h4>转换线索 <small>${param.fullname}${param.appleation}-${param.company}</small></h4>
	</div>
	<div id="create-customer" style="position: relative; left: 40px; height: 35px;">
		新建客户：${param.company}
	</div>
	<div id="create-contact" style="position: relative; left: 40px; height: 35px;">
		新建联系人1：${param.fullname}${param.appleation}
	</div>
